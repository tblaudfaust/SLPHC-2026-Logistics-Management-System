...
def office_items_summary(
    ...
    caller's store (national, region, or — with `district_id` — one
    district), not a curated subset: this single table is meant to answer
    "what's in this store" in full, tablets and Starlink kits included
    alongside laptops and stationery, rather than splitting "office
    supplies" from "census fleet" into separate glances. A category with
    nothing on hand in scope is omitted rather than shown as a zero row.
    For a quantity-tracked category, "available" is the same as "total":
    all on-hand quantity in a warehouse is available by definition, there's
    no separate allocation sub-state for consumables the way there is for
