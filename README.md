...
## Fuel Management module (2026-09-10)

Added a full fuel-accountability module — allocation, request,
two-level approval, issuance, receipt acknowledgement, reconciliation,
vouchers, and a stock ledger — for vehicles, motorcycles, generators,
and Starlink field kits, built on top of this app's existing platform
(RBAC, notifications, audit, geography scoping, the report engine)
rather than as a separate system.

**Data model:** Vehicles and Generators extend the existing `Asset`
table the same way `StarlinkKit` does (asset tag, status, location all
reused); motorcycles are a `vehicle_type` on `Vehicle`, not a separate
table. `FuelStockTransaction` is an append-only ledger mirroring
`InventoryTransaction` — on-hand balance is always computed live via
`SUM(quantity)`, never a stored/mutable summary row. Fuel references
(`SLPHC-FUEL-2026-REQ-000001` etc.) are generated by a
`SELECT ... FOR UPDATE`-locked counter per entity type, the same
collision-safe pattern `asset_service.generate_asset_tag` uses.

**Approval chain:** matches Statistics Sierra Leone's actual structure
rather than the brief's generic multi-tier version — every request
goes to the Director for level-1 review, then the Statistician General
gives final level-2 approval, with the Deputy Statistician General
able to approve in the SG's place (both roles hold the same
`fuel.approve_final` permission, so whoever acts first is the recorded
approver; no separate delegation feature needed). Business rules
enforced in the service layer: a request cannot exceed its
allocation's remaining balance; level-2 approved quantity can be lower
than requested (but not higher); issued quantity cannot exceed the
level-2 approved quantity; level 2 cannot be recorded before level 1.

**Scope delivered this pass** (the original spec named ~19 modules,
~14 alert types, ~21 report types, and 8 dashboard charts — this
build covers the core accountability chain end-to-end, not every item
in that catalogue):
- Core workflow: allocations, requests, 2-level approvals, issuance,
  receipt acknowledgement, reconciliation.
- Vehicles, Generators, Fuel Stations, Fuel Vouchers (basic
  create/list/status — not yet integrated into reconciliation).
- Fuel stock ledger with live-computed on-hand balances per depot.
- 2 Celery Beat alerts (low allocation balance, overdue reconciliation),
  reusing the existing notification/dedupe pattern.
- 4 reports registered into the existing report engine (Fuel
  Allocation, Fuel Request, Fuel Issuance, Regional Fuel Consumption).
- A new `FuelPage.tsx` with 8 tabs (Dashboard, Requests & Approvals,
  Vehicles, Generators, Fuel Stations, Allocations, Vouchers, Stock)
  and 2 dashboard charts, using `recharts` — the app's first charting
  dependency.
- 11 new roles seeded (Statistician General, Deputy Statistician
  General, Directorate Head, Fuel Manager, Finance Officer, Fuel
  Issuing Officer, Driver, Generator Operator, Regional Coordinator,
  District Census Officer, Monitoring Officer).

**Not built this pass:** MFA, deep voucher-to-reconciliation
integration, and the remainder of the brief's report/alert/chart
catalogue — all straightforward fast-follows using the same extension
points (`REPORT_DEFINITIONS`, the Celery Beat alert pattern) proven
here.

Verified live end-to-end on production: registered vehicle
`SLPHC26-VEH-000001`, created a 100L diesel allocation, submitted a
30L request (confirmed an 80L over-request against the resulting 70L
remaining balance is correctly rejected with 400), approved at level 1
(30L) then level 2 (25L — deliberately lower, confirming independent
per-level amounts), confirmed issuing 30L against the 25L approval is
correctly blocked, issued 25L, acknowledged receipt, reconciled at 24L
consumed (1L balance), and confirmed the request's final status
(`RECONCILED`), the stock ledger (-25L at Freetown-Central-Store), and
the dashboard KPIs (75L remaining balance, Le137.50 expenditure) all
reconcile correctly. Browser-verified all 8 `FuelPage.tsx` tabs render
correctly against this same data, including the request-creation
dialog's live "70L remaining" allocation display and fully populated
region/district/activity dropdowns.
